...
const CbomPage = () => {
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <div className="space-y-2 mt-2">
            {cipherData.map((item, i) => (
              <div key={i} className="flex items-center gap-2">
                <div
                  className={`flex-1 text-xs font-mono px-2 py-1.5 rounded truncate ${isWeakCipher(item.cipher) ? 'bg-danger/20 text-danger border border-danger/30' : 'bg-[#1f2937] text-gray-300 border border-border/50'}`}
                  title={item.cipher}
                >
                  {item.cipher}
                </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="glass-panel p-6 flex-1">
            ...
            <div className="space-y-2">
              {caData.map((ca, i) => (
                <div key={i} className="flex items-center gap-2">
                  <div className="flex-1 bg-[#1f2937]/50 px-3 py-1.5 rounded border border-border/50">
                    <span className="text-sm text-gray-300 truncate block" title={ca.name}>{ca.name}</span>
                  </div>
                  <span
          ...
          <div className="glass-panel p-4">
            <h3 className="text-xs font-bold text-textMuted uppercase tracking-wider mb-2">Encryption Protocols</h3>
      ...
      {/* CBOM Detail Table */}
      <div className="glass-panel p-6">
        <h3 className="text-sm font-bold text-textMuted uppercase tracking-wider mb-4 border-b border-border pb-3">
              ...
              {scans.map((scan) => {
                ...
                return (
                  <tr key={scan._id} className="border-b border-border/40 hover:bg-[#1f2937]/30 transition-colors">
                    ...
                    <td className="py-2 px-3">
                      <span className={`text-xs font-mono px-2 py-0.5 rounded ${isWeak ? 'bg-danger/20 text-danger border border-danger/30' : 'bg-[#1f2937] text-gray-300'}`}>
                        {scan.tlsConfiguration?.cipherSuite || 'N/A'}
                      </span>
                    </td>
                    <td className="py-2 px-3 text-sm text-gray-300">{caShort}</td>
                    <td className="py-2 px-3 text-xs text-gray-400 font-mono">{scan.certificateDetails?.signatureAlgorithm || 'N/A'}</td>
