  ...
  const renderMarkdown = (text) => {
    ...
    lines.forEach((line, i) => {
      ...
      if (trimmed.startsWith('### ')) {
        elements.push(<h4 key={i} className="text-md font-bold text-textMain mt-4 mb-2">{renderInline(trimmed.slice(4))}</h4>);
      } else if (trimmed.startsWith('## ')) {
        elements.push(<h3 key={i} className="text-lg font-bold text-textMain mt-4 mb-2">{renderInline(trimmed.slice(3))}</h3>);
      } else if (trimmed.startsWith('# ')) {
        elements.push(<h2 key={i} className="text-xl font-bold text-textMain mt-4 mb-2">{renderInline(trimmed.slice(2))}</h2>);
      }
      // Bullet points
        ...
        elements.push(
          <div key={i} className="flex gap-2 ml-4 mb-1">
            ...
            <span className="text-textMuted text-sm">{renderInline(trimmed.slice(2))}</span>
          </div>
        );
      }
      // Numbered list
        ...
        elements.push(
          <div key={i} className="flex gap-2 ml-4 mb-1">
            ...
            <span className="text-textMuted text-sm">{renderInline(content)}</span>
          </div>
        );
      }
      // Regular paragraph
      else {
        elements.push(<p key={i} className="text-textMuted text-sm mb-1">{renderInline(trimmed)}</p>);
      }
    });
  ...
  const renderInline = (text) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return <strong key={i} className="text-textMain font-semibold">{part.slice(2, -2)}</strong>;
      }
      // Inline code 
    ...
    return (
      <ul className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
        {ciphers.map((group, idx) => {
          let badge = '';
          let badgeClass = "text-textMuted border-gray-600 bg-gray-600/10";
          ...
          return (
            <li key={idx} className={`px-3 py-2 rounded text-sm font-mono border flex items-center justify-between transition-colors ${isRejected ? 'bg-[#1f2937]/30 border-border/30 opacity-80' : 'bg-[#1f2937]/50 border-border/50 text-textMuted'}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="glass-panel p-4 flex items-center justify-between border-t-2 border-t-primary">
          <div>
            ...
            <p className="text-3xl font-bold mt-1 text-textMain">{totalScans}</p>
          </div>
        ...
        <div className="glass-panel p-4 flex items-center justify-between border-t-2 border-t-secondary">
          <div>
            ...
            <p className="text-3xl font-bold mt-1 text-textMain">{pqcReadyCount}</p>
          </div>
        ...
        <div className="glass-panel p-4 flex items-center justify-between border-t-2 border-t-danger">
          <div>
            ...
            <p className="text-3xl font-bold mt-1 text-textMain">{highRiskCount}</p>
          </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-panel border border-border rounded-lg px-4 py-3 text-textMain neon-border-focus focus:outline-none transition-all placeholder:text-textMuted"
                    required
                  />
                  {loading && (
            ...
            <div className={`glass-panel p-6 border-l-4 ${currentScan.pqcSupport.statusLabel === 'Non-PQC Ready' ? 'border-l-danger' :
              ...
              <div className="mb-6 flex flex-col items-center justify-center p-6 bg-[#1f2937]/50 rounded-xl border border-border">
                ...
                <div className="flex items-center gap-4 mt-4 bg-primary/20 px-6 py-4 rounded-lg border border-primary/30 w-full justify-center">
                  <p className="text-5xl font-extrabold text-textMain">{currentScan.securityScore}<span className="text-3xl text-textMuted">/1000</span></p>
                ...
                {currentScan.assetInventory.domains.slice(0, 5).map((d, i) => (
                  <div key={i} className="bg-[#1f2937]/50 rounded-lg p-3 border border-border flex flex-col gap-2 transition-colors hover:bg-[#1f2937]">
                    <div className="flex justify-between items-center">
                      <span className="font-mono text-sm text-textMain truncate pr-2 flex items-center gap-1.5"><Globe size={12} className="text-textMuted" /> {d.domain}</span>
              ...
              {currentScan && currentScan.pqcSupport && (
                <div className="mt-4 grid grid-cols-4 gap-2 text-center border-t border-border/50 pt-4">
                  <div className="bg-[#1f2937]/30 rounded p-1 overflow-hidden">
                    ...
                    <p className="text-sm font-bold text-textMain mt-0.5">{currentScan.pqcSupport.rawResults ? Object.keys(currentScan.pqcSupport.rawResults).length : 43}</p>
                  </div>
                  <div className="bg-[#1f2937]/30 rounded p-1 overflow-hidden">
              ...
              <div className="glass-panel p-6 flex flex-col">
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <div className="bg-[#1f2937]/30 p-3 rounded-lg border border-border">
                    ...
                    <p className="text-xs font-mono text-textMain mt-1 break-all">{currentScan.tlsConfiguration?.cipherSuite || 'N/A'}</p>
                  ...
                  <div className="bg-[#1f2937]/30 p-3 rounded-lg border border-border">
                    ...
                    <p className="text-sm font-semibold text-textMain mt-1 truncate" title={currentScan.certificateDetails.issuer}>
                      {(() => {
                  ...
                  <div className="bg-[#1f2937]/30 p-3 rounded-lg border border-border">
                    ...
                    <p className="text-xs font-mono text-textMuted mt-1">From: {currentScan.certificateDetails.validFrom}</p>
                    <p className={`text-xs font-mono ${isExpired(currentScan.certificateDetails.validUntil) ? 'text-danger font-bold' : 'text-textMuted'}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                {['SSLv3', 'TLSv1.0', 'TLSv1.1', 'TLSv1.2', 'TLSv1.3'].map((ver) => {
                  ...
                  return (
                    <div
                      key={ver}
                      className={`p-4 rounded-lg border text-center transition-all ${isSupported
                        ? isDeprecated
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {history.map((record) => (
                ...
                >
                  <td className="py-3 px-4 font-medium text-textMain hover:text-primary transition-colors">{record.target}</td>
                  ...
                  <td className="py-3 px-4 text-right">
                    ...
                    <span className="text-sm text-textMain font-medium ml-2" title="Total Scanned">
                      {(record.pqcSupport?.supportedAlgorithms?.length || 0) + (record.pqcSupport?.partiallySupportedAlgorithms?.length || 0) + (record.pqcSupport?.rejectedAlgorithms?.length || 0)}
                    </span>
                  </td>
                </tr>
              ))}
