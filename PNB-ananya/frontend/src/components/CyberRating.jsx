  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Enterprise Cyber Rating</h1>
          ...
          return (
            <div key={name} className={`glass-panel p-4 flex items-center justify-between border-t-2 ${cfg.border}`}>
              <div>
                ...
                <p className="text-3xl font-bold mt-1 text-white">{tierCounts[name]}</p>
      ...
      {selectedScan && (
        <div className={`glass-panel p-6 border-l-4 ${config.border}`}>
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-bold text-white">{selectedScan.target}</h3>
          ...
          <div className="flex items-center justify-center mb-6 p-6 bg-[#1f2937]/50 rounded-xl border border-border">
            <p className="text-5xl font-extrabold text-white">{selectedScan.securityScore}<span className="text-3xl text-textMuted">/1000</span></p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-[#1f2937]/30 p-4 rounded-lg border border-border">
              ...
              <p className="text-sm text-white font-medium">{rating.securityLevel}</p>
            </div>
            <div className="bg-[#1f2937]/30 p-4 rounded-lg border border-border">
              ...
              <p className="text-sm text-white font-medium">{rating.complianceCriteria}</p>
            </div>
            <div className="bg-[#1f2937]/30 p-4 rounded-lg border border-border">
              ...
              <p className="text-sm text-white font-medium">{rating.priorityAction}</p>
            </div>
          </div>
        </div>
      )}
                  ...
                  >
                    ...
                    <td className="py-2 px-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-mono text-white">{scan.securityScore}</span>
                      </div>
                    </td>
                    <td className="py-2 px-3">
                      <span className={`px-2 py-1 rounded text-xs font-semibold ${c.bg} ${c.color} border ${c.border}`}>{t}</span>
                    </td>
                    <td className="py-2 px-3 text-sm text-gray-300">{r.securityLevel}</td>
                    <td className="py-2 px-3">
              ...
              {[
                ...
                return (
                  <tr key={i} className="border-b border-border/40">
                    ...
                    <td className="py-2 px-3 text-sm text-white font-mono">{row.range}</td>
                    <td className="py-2 px-3 text-sm text-gray-300">{row.level}</td>
                    <td className="py-2 px-3 text-xs text-gray-400">{row.criteria}</td>
                    <td className="py-2 px-3 text-xs text-gray-400">{row.action}</td>
                  </tr>
                );
              })}
            </tbody>
