        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="glass-panel p-4 flex items-center justify-between border-t-2 border-t-secondary">
            <div>
              ...
              <p className="text-2xl font-bold mt-1 text-white">{uniqueDomains}</p>
            </div>
          ...
          <div className="glass-panel p-4 flex items-center justify-between border-t-2 border-t-[#d97706]">
            <div>
              ...
              <p className="text-2xl font-bold mt-1 text-white">{avgScore}/1000</p>
            </div>
          ...
          <div className="glass-panel p-4 flex items-center justify-between border-t-2 border-t-danger">
            <div>
              ...
              <p className="text-2xl font-bold mt-1 text-white">{highRiskCount}</p>
            </div>
    ...
    <div className="flex flex-col gap-6" ref={reportRef}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">{currentReportConfig.title}</h1>
                  ...
                  return (
                    <div key={tier}>
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-sm font-mono text-white">{count} <span className="text-textMuted">({totalScans > 0 ? Math.round((count / totalScans) * 100) : 0}%)</span></span>
                      </div>
          ...
          <div className="glass-panel p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-[#1f2937]/30 p-4 rounded-lg border border-danger/20">
                ...
                <p className="text-sm text-gray-300">{highRiskCount > 0 ? `${highRiskCount} assets classified as Non-PQC Ready require urgent cryptographic upgrades to mitigate HNDL attack vectors.` : 'All assets meet minimum PQC readiness. Continue monitoring.'}</p>
              </div>
              <div className="bg-[#1f2937]/30 p-4 rounded-lg border border-warning/20">
                ...
                <p className="text-sm text-gray-300">Implement hybrid key exchange (X25519+ML-KEM) across {tierCounts['Tier-2 Standard'] + tierCounts['Tier-3 Legacy']} standard/legacy-tier assets to achieve baseline PQ safety.</p>
              </div>
              <div className="bg-[#1f2937]/30 p-4 rounded-lg border border-secondary/20">
                ...
                <p className="text-sm text-gray-300">Complete NIST PQC migration across all {uniqueDomains} domains. Target Tier-1 Elite rating (score &gt;700) for all public-facing assets.</p>
              </div>
            </div>
          </div>
        </>
              ...
              {topCiphers.map(([cipher, count], i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="flex-1 bg-[#1f2937]/50 rounded-lg overflow-hidden h-8 relative border border-border/50">
                    ...
                    <span className="absolute inset-0 flex items-center px-3 text-xs font-mono text-gray-300 truncate">{cipher}</span>
                  </div>
                ...
                <tbody>
                  {scans.map((scan, i) => (
                    <tr key={scan._id} className="border-b border-border/40 hover:bg-[#1f2937]/30 transition-colors">
                      ...
                      <td className="py-2 px-3 text-sm font-medium text-white">{scan.target}</td>
                      <td className="py-2 px-3 text-xs text-gray-400">{new Date(scan.scanDate).toLocaleDateString()}</td>
                      ...
                      <td className="py-2 px-3 text-xs font-mono text-gray-300">{scan.tlsConfiguration?.cipherSuite || 'N/A'}</td>
                      <td className="py-2 px-3 text-xs text-gray-400">{scan.tlsConfiguration?.protocol || 'N/A'}</td>
                      <td className="py-2 px-3">
      ...
      {selectedReport === 'compliance' && (
        <>
          ...
          <div className="glass-panel p-6 border-l-4 border-l-[#d97706]">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-bold text-white">Compliance Assessment Summary</h3>
                  ...
                  {scans.map((scan, i) => {
                    ...
                    return (
                      <tr key={scan._id} className="border-b border-border/40 hover:bg-[#1f2937]/30 transition-colors">
                        <td className="py-2 px-3 text-sm text-textMuted">{i + 1}</td>
                        <td className="py-2 px-3 text-sm font-medium text-white">{scan.target}</td>
                        <td className="py-2 px-3 text-xs text-gray-400">{new Date(scan.scanDate).toLocaleString()}</td>
                        <td className="py-2 px-3">
                          <span className={`text-sm font-mono font-bold ${scan.securityScore > 700 ? 'text-primary' : scan.securityScore >= 400 ? 'text-warning' : 'text-danger'}`}>
                            {scan.securityScore}
                          </span>
                        </td>
                        <td className="py-2 px-3 text-xs text-gray-300">{tier}</td>
                        <td className="py-2 px-3 text-xs text-gray-400">{scan.tlsConfiguration?.protocol || 'N/A'}</td>
                        <td className="py-2 px-3 text-xs font-mono text-gray-300">{scan.certificateDetails?.publicKeySize ? `${scan.certificateDetails.publicKeySize}-bit` : 'N/A'}</td>
                        <td className="py-2 px-3">
            ...
            <div className="space-y-3">
              {highRiskCount > 0 && (
                <div className="flex gap-3 p-3 bg-danger/5 rounded-lg border border-danger/20">
                  ...
                  <div>
                    <p className="text-sm font-medium text-white">Upgrade {highRiskCount} Non-PQC Ready assets</p>
              ...
              <div className="flex gap-3 p-3 bg-warning/5 rounded-lg border border-warning/20">
                ...
                <div>
                  <p className="text-sm font-medium text-white">Establish continuous monitoring</p>
              ...
              <div className="flex gap-3 p-3 bg-secondary/5 rounded-lg border border-secondary/20">
                ...
                <div>
                  <p className="text-sm font-medium text-white">Certificate rotation plan</p>
