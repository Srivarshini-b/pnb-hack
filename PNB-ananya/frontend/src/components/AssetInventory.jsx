            ...
            <tbody>
              {domains.length > 0 ? domains.map((row, idx) => (
                <tr key={idx} className="border-b border-border/50 hover:bg-[#1f2937]/30 transition-colors text-sm">
                  ...
                  <td className="py-3 px-4 text-gray-300">{row.registrar}</td>
            ...
            <tbody>
              {certificates.length > 0 ? certificates.map((row, idx) => (
                <tr key={idx} className="border-b border-border/50 hover:bg-[#1f2937]/30 transition-colors text-sm">
                  ...
                  <td className="py-3 px-4 text-white font-medium">{row.commonName}</td>
                  <td className="py-3 px-4 text-gray-300">{row.company}</td>
            ...
            <tbody>
              {ips.length > 0 ? ips.map((row, idx) => (
                <tr key={idx} className="border-b border-border/50 hover:bg-[#1f2937]/30 transition-colors text-sm">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex gap-1 flex-wrap">
                      {row.ports ? row.ports.split(',').map(port => (
                        <span key={port} className="bg-[#1f2937] text-gray-300 px-2 py-0.5 rounded text-xs border border-border">
                          {port.trim()}
                        </span>
                      )) : 'N/A'}
                    </div>
                  ...
                  <td className="py-3 px-4 text-gray-300">{row.netname}</td>
          ...
          <div className="bg-[#1f2937] border border-border rounded-xl shadow-2xl w-full max-w-5xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-border bg-[#111827]">
              ...
              <button
                ...
                className="p-2 bg-[#1f2937] hover:bg-danger/20 border border-transparent hover:border-danger/30 rounded-full transition-all text-gray-400 hover:text-danger"
