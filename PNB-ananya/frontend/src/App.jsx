  ...
  return (
    <div className="min-h-screen bg-background text-textMain flex flex-col font-sans">
      ...
      <header className="glass-panel rounded-none border-t-0 border-l-0 border-r-0 border-b-primary/30 px-6 py-4 flex flex-col md:flex-row items-center justify-between sticky top-0 z-50 gap-4 md:gap-0">
        ...
        <div className="flex bg-[#1f2937]/80 rounded-lg p-1 border border-border">
          <button
            onClick={() => setActiveView('dashboard')}
            className={`flex items-center gap-2 px-4 py-2 rounded-md transition-all text-sm font-medium ${activeView === 'dashboard' ? 'bg-primary/20 text-primary border border-primary/30' : 'text-textMuted hover:text-white'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-md transition-all text-sm font-medium ${activeView === 'cbom' ? 'bg-[#8b5cf6]/20 text-[#8b5cf6] border border-[#8b5cf6]/30' : 'text-textMuted hover:text-white'}`}
          >
            <FileText size={16} /> CBOM
          </button>
          <button
            onClick={() => setActiveView('rating')}
            className={`flex items-center gap-2 px-4 py-2 rounded-md transition-all text-sm font-medium ${activeView === 'rating' ? 'bg-warning/20 text-warning border border-warning/30' : 'text-textMuted hover:text-white'}`}
          >
            <Shield size={16} /> Cyber Rating
          </button>
          <button
            onClick={() => setActiveView('reporting')}
            className={`flex items-center gap-2 px-4 py-2 rounded-md transition-all text-sm font-medium ${activeView === 'reporting' ? 'bg-[#d97706]/20 text-[#d97706] border border-[#d97706]/30' : 'text-textMuted hover:text-white'}`}
          >
